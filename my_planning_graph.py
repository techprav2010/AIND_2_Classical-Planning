...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        layers.ActionNode
        """
        negatedEffectsA = set([~effectA for effectA in actionA.effects])  # set of all negated effects A
        sharedSize = len(actionB.effects.intersection(negatedEffectsA)) ##count of intersection = (neg ActionA.Effects + actionB.Effects)

        # print("_inconsistent_effects sharedSize " , sharedSize,
        #       ", effectsASize ", len(actionA.effects), ", preconditionsA ", len(actionA.preconditions),
        #       ", effectsBSize ", len(actionB.effects), ", preconditionsB ", len(actionB.preconditions),
        #       ", A e ", actionA.effects, " , p ", actionA.preconditions,
        #       ", B e ", actionB.effects, " , p ", actionB.preconditions  )
    ...
    def _interference(self, actionA, actionB):
        ...
        layers.ActionNode
        """
        negatedEffectsA = set([~effectA for effectA in actionA.effects])  # set of all negated actionA.effects
        negatedPreconditionsA = set([~preconditionA for preconditionA in actionA.preconditions])  # set of all negated actionA.preconditions

        sharedSize = len(actionB.effects.intersection(negatedPreconditionsA))  #count of intersection = (neg ActionA.preconditions + actionB.effects)
        if (sharedSize == 0):
            sharedSize += len(actionB.preconditions.intersection(negatedEffectsA))##count of intersection = (neg ActionA.Effects + actionB.preconditions)

        # print("_interference sharedSize ", sharedSize, #", sharedPreconditionsSize ", sharedPreconditionsSize,
        #       ", effectsASize ", len(actionA.effects), ", preconditionsA ", len(actionA.preconditions),
        #       ", effectsBSize ", len(actionB.effects), ", preconditionsB ", len(actionB.preconditions),
        #       ", A e ", actionA.effects, " , p ", actionA.preconditions,
        #       ", B e ", actionB.effects, " , p ", actionB.preconditions
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        # any true mutex from (inner  loop for each actionB -> precondition ; outer loop for each actionA -> precondition)
        mutexList = any([self.parent_layer.is_mutex(itemA, itemB)
                         for itemA in actionA.preconditions
                         for itemB in actionB.preconditions])

        # print("_competing_needs mutexList ", mutexList, ", mutexList ",
        #       len(mutexList),
        #       ", effectsASize ", len(actionA.effects), ", preconditionsA ", len(actionA.preconditions),
        #       ", effectsBSize ", len(actionB.effects), ", preconditionsB ", len(actionB.preconditions),
        #       ", A e ", actionA.effects, " , p ", actionA.preconditions,
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        layers.BaseLayer.parent_layer
        """
        # all true mutex from (inner loop for each parentsB[literalB] -> precondition; outer loop for each parentsA[literalA] -> precondition)
        return all(self.parent_layer.is_mutex(itemA, itemB)
                   for itemA in self.parents[literalA]
                   for itemB in self.parents[literalB])
...
class PlanningGraph:
    ...
    # def _level_cost(self, g_literal):
    #     """Helper for other level-cost related heuristics -- ychen"""
    #     for index, layer in enumerate(self.literal_layers):
    #         if g_literal in layer:
    #             return index
    ...
    def h_levelsum(self):
        ...
        self.fill()
        levelSum = 0
        for goal in self.goal: # cost for each goal
            for index,layer in enumerate(self.literal_layers):
                if goal in layer: #found a goal at this layer, find the next goal
                    levelSum += index
                    break
        return levelSum
    ...
    def h_maxlevel(self):
        ...
        for goal in self.goal:
            for cost, layer in enumerate(self.literal_layers):
                if goal in layer:
                    max_goal_cost = max(cost, max_goal_cost)
                    break
        return max_goal_cost
    ...
    def h_setlevel(self):
        ...
        while not self._is_leveled:
            layer = self.literal_layers[layer_index]
            if self.goal.issubset(layer) and not any( layer.is_mutex(goalA, goalB) and goalA != goalB for goalA in  self.goal for goalB in  self.goal):
                return layer_index
            # if self.goal.issubset(layer) and not any( layer.is_mutex(goalA, goalB) and goalA != goalB for goalA in  layer for goalB in  layer):
            #     return layer_index
            self._extend()
            layer_index += 1
